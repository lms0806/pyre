...
fn collect_guards_and_vars(inputargs: &[InputArg], ops: &[Op]) -> (Vec<GuardExit>, u32) {
    ...
    for op in ops {
        if op.pos != OpRef::NONE && !op.pos.is_constant() && op.pos.0 + 1 > max_var {
            max_var = op.pos.0 + 1;
        }
        if op.opcode == OpCode::Label {
            for &a in &op.args {
                if a != OpRef::NONE && !a.is_constant() && a.0 + 1 > max_var {
                    max_var = a.0 + 1;
                }
            }
        }
    ...
    for op in ops {
        match op.opcode {
            ...
            | OpCode::CallReleaseGilF => {
                ...
                if !OpRef(vi).is_constant() && !is_void {
                    sink.local_get(0);
                    sink.i64_load(mem64(CALL_RESULT_OFS));
                    sink.local_set(1 + vi);
                }
