...
mod slice_x2_probe {
    //! Instrumentation: count how often the host-loop
...
/// Helper for the `fail_descrs: Box<[DescrRef]>` storage.
/// Every entry in a `fail_descrs` collection
/// implements `FailDescr` by construction (codegen pushes singleton
/// `Arc<CraneliftFailDescr>` instances upcast to `DescrRef`); this
/// helper makes the `as_fail_descr()` downcast explicit at every
/// FailDescr-method callsite so the `Box<[DescrRef]>` field can drive
/// trait-method dispatch through the metainterp's `ResumeGuardDescr`
/// without backend-wrapper indirection.
#[inline]
fn as_fd(d: &DescrRef) -> &dyn FailDescr {
    d.as_fail_descr()
...
/// it via the identity-preservation path.
...
fn attached_descr_ptrs_with_fallbacks(
    ...
    // Singletons are now `DescrRef = Arc<dyn Descr>`.
...
thread_local! {
    /// Per-thread `TargetToken._ll_loop_code` registry. Same TODO
    /// rationale as `CALL_ASSEMBLER_TARGETS` — pyre's u64-keyed
...
fn deadframe_layout(frame: &DeadFrame) -> Option<FailDescrLayout> {
    ...
    let fail_index = fd.fail_index();
    let trace_id = fd.trace_id();
    let mut layout = fail_descr_layout(&descr_ref, fail_index, trace_id);

    // If a `wrap_call_assembler_deadframe_with_caller_prefix`
    // call has stashed a caller-prefix layout on this deadframe, prepend
    // it to the descr's own recovery layout (the same composition the
    // overlay descr machinery used to bake at wrap time).  Replays the
    // PyPy `assembler_call_helper` parent-frame chain reconstruction
    // (`pyjitpl.MetaInterp.framestack` walk) at deopt time instead of
    // pre-baking.
    if let Some(caller_layout) = &jf.call_assembler_caller_layout {
        let inner = layout
            .recovery_layout
            .clone()
            .or_else(|| deadframe_recovery_layout_for_call_assembler(&layout));
        if let Some(inner) = inner {
            let prefixed = inner.prefixed_by(Some(caller_layout));
            layout.frame_stack = Some(prefixed.frames.clone());
            layout.recovery_layout = Some(prefixed);
        }
    }
    Some(layout)
}
...
fn wrap_call_assembler_deadframe_with_caller_prefix(mut frame: DeadFrame) -> DeadFrame {
    ...
    if jf.call_assembler_caller_layout.is_some() {
        // Nested wrap on the same deadframe would compose caller prefixes
        // incorrectly; the dispatch design guarantees a single wrap call
        // per CALL_ASSEMBLER deadframe interception.
        panic!(
            "wrap_call_assembler_deadframe_with_caller_prefix: deadframe \
...
/// at a time.  The struct itself disappears together with
/// `execute_with_inputs` once the dispatch loop collapses to a
/// single-call `execute_token` (PyPy `llmodel.execute_token`).
#[derive(Clone)]
...
thread_local! {
    ...
    /// RegisteredLoopTarget` map is a TODO required
...
fn unregister_bridge_call_assembler_expectations(bridge: &BridgeData) {
    ...
    for descr in &bridge.fail_descrs {
        if let Some(child_bridge) = fail_descr_bridge_ref(as_fd(descr)) {
            unregister_bridge_call_assembler_expectations(&child_bridge);
        }
    }
}

fn unregister_call_assembler_bridge_tree(fail_descrs: &[DescrRef]) {
...
fn register_call_assembler_target(
    ...
    *clt._ll_initial_locs.lock() = (0..compiled.num_inputs).map(|i| (i as i32) * 8).collect();
    ...
    {
        let result_type = as_fd(finish_descr)
            .fail_arg_types()
            .first()
            .copied()
            .unwrap_or(Type::Void);
...
fn call_assembler_finish_or_blackhole_deadframe(mut frame: DeadFrame) -> Option<i64> {
    ...
    //
    // The blackhole hook recovers its descr via
    // `Backend::fail_descr_arc_from_addr` → `recover_fail_descr_cell`,
...
fn execute_registered_loop_target(target: &RegisteredLoopTarget, inputs: &[i64]) -> DeadFrame {
    ...
    loop {
        ...
        if let Some(bridge) = fail_descr_bridge_ref(fail_descr_fd) {
            ...
            // Route through rebuild_state_after_failure_dispatch
    ...
    for (op_idx, op) in ops.iter().enumerate() {
        ...
        if op_dereferences_first_arg(op.opcode) {
            ...
            let bound = arg.is_none()
                || arg.is_constant()
                || arg.to_opref().inline_const_bits().is_some()
                || constants.contains_key(&arg.to_opref().raw())
...
fn build_type_overrides(
    ...
    for (descr_idx, jump_idx) in &jumps_by_descr {
        ...
        for (i, label_arg) in label_op.getarglist().iter().enumerate() {
            ...
            let Some(jump_arg) = jump_op.getarglist().get(i).map(|a| a.to_opref()) else {
                continue;
            };
            if jump_arg.is_none() {
                continue;
            ...
            let label_type = lookup(label_arg.to_opref(), label_idx, Type::Ref, &overrides);
            if jump_type != label_type {
                overrides.insert(label_arg.to_opref().raw(), jump_type);
                op_def_positions.insert(label_arg.to_opref().raw(), label_idx);
            }
        }
    }
...
/// dispatch cell.  Returns `None` when no bridge has been attached.
/// Runs on `&dyn FailDescr` directly.
///
...
/// emission slot.  Wrap the value in `Arc<dyn Any>` so the
/// `FailDescr::set_trace_info_any` trait method can carry it without
/// depending on `majit-backend`'s concrete type.  Silently no-ops for
/// descrs whose `set_trace_info_any` is the trait default (synthetic
...
/// slot on the Resume-family descr.  Dispatches via
...
/// `fail_descrs`.  FINISH descrs are now the static
...
///   pointer (`collect_guards`).  `recover_fail_descr_cell`
    ...
    fn do_compile(
        ...
        for (op_idx, op) in ops.iter().enumerate() {
            ...
            for arg in op.getarglist().iter().chain(
                ...
                if !arg.is_none()
                    && !declared_vars.contains(&arg.to_opref().raw())
                    && !constants.contains_key(&arg.to_opref().raw())
                {
                    declared_vars.insert(arg.to_opref().raw());
                    if debug_declares {
                        eprintln!(
                    ...
                    var_types.insert(arg.to_opref().raw(), cl_type_for_opref(arg.to_opref()));
                }
            }
        }
        ...
        for op in ops {
            ...
            for arg in op.getarglist().iter() {
                ...
                if debug_declares {
                    // Independent debug toggle — not gated by MAJIT_LOG.
                    eprintln!("[jit][declare] label-arg var{}", arg.to_opref().raw());
                }
                var_types.insert(arg.to_opref().raw(), cl_type_for_opref(arg.to_opref()));
            }
        }
            ...
            match op.opcode {
                ...
                OpCode::GuardSubclass => {
                    ...
                    let loc_check_against_class =
                        lookup_const_i64(&constants, op.arg(1).to_opref()).unwrap_or_else(|| {
                            panic!(
                ...
                OpCode::CallMallocNursery => {
                    ...
                    if use_inline {
                        ...
                        let size_val =
                            op.arg(0).to_opref().inline_const_bits().unwrap_or_else(|| {
                                constants
                                    .get(&op.arg(0).to_opref().raw())
                                    .copied()
                                    .unwrap_or(op.arg(0).to_opref().raw() as i64)
                            });
                ...
                OpCode::GcStore => {
                    ...
                    if op.num_args() >= 4 {
                        ...
                        if std::env::var_os("MAJIT_CL_GCSTORE_LOG").is_some() {
                            let offset_dbg = lookup_const_i64(&constants, op.arg(1).to_opref());
                            eprintln!(
        ...
        if std::env::var_os("MAJIT_LOG").is_some() {
            let fail_descr_preview: Vec<(u32, usize)> = fail_descrs
                ...
                .map(|descr| {
                    (
                        as_fd(descr).fail_index_per_trace(),
        ...
        // TODO removed: singleton FINISH descrs (the
...
fn collect_guards(
    ...
    for (op_idx, op) in ops.iter().enumerate() {
        ...
        let (fail_arg_refs, fail_arg_types) = if is_finish || is_external_jump {
            let refs: Vec<OpRef> = op.getarglist().iter().map(|a| a.to_opref()).collect();
        ...
        // Descr is now `DescrRef` directly — the metainterp
        // `Arc<DoneWithThisFrameDescr*>` /
        ...
        let descr: DescrRef = if is_finish {
            ...
            // singletons are now the metainterp Arc directly, no
            // CraneliftFailDescr wrapper layered on top.
            //
...
impl majit_backend::Backend for CraneliftBackend {
    ...
    fn compile_bridge(
        ...
        // the prior `previous_tokens` scan was a TODO compensating
        ...
        // caller_recovery_layout is now passed in by the
    ...
    fn migrate_bridges(&self, old_token: &JitCellToken, new_token: &JitCellToken) {
        ...
        for old_fd in &old.fail_descrs {
            let Some(b) = fail_descr_bridge_ref(as_fd(old_fd)) else {
                continue;
            };
...
// `majit_backend::call_stub::bh_call_i_dispatch` so dynasm shares
// the same dispatch.
...
mod tests {
    ...
    fn test_compiled_fail_descr_layouts_include_backend_recovery_layout() {
        ...
        // Backend no longer caches per-fail-descr recovery
    ...
    fn test_compiled_bridge_recovery_layouts_inherit_source_guard_caller_frames() {
        ...
        assert!(
            ...
            "bridge guard fail_descr recovery_layout is metainterp-owned"
        );
    ...
    fn test_nested_bridge_compilation_uses_source_trace_fail_descr_tree() {
        ...
        // Backend no longer caches recovery_layout; layout flows
        ...
        // Backend no longer caches recovery_layout; the bridge
        ...
        assert!(
            ...
            "nested bridge guard fail_descr recovery_layout is metainterp-owned"
        );
    ...
    fn test_guard_failure_counting() {
        ...
        for i in 1..=5 {
            ...
            assert_eq!(as_fd(descr).fail_count(), i);
        }
        ...
        assert_eq!(as_fd(guard_descr).fail_count(), 5);
    }
    ...
    fn test_call_assembler_supports_direct_self_recursive_dispatch() {
        ...
        let guard_fd = as_fd(guard_descr);
    ...
    fn test_all_guards_have_recovery_layout() {
        ...
        // Backend no longer caches a per-descr recovery
        ...
        for (idx, layout) in layouts.iter().enumerate() {
            assert!(
                layout.recovery_layout.is_none(),
                "fail_descr[{idx}] recovery_layout must be None at the backend boundary"
            );
            if !layout.is_finish {
    ...
    fn test_frame_layout_includes_slot_types() {
        ...
        // Per-fail-descr recovery_layout (and the derived
