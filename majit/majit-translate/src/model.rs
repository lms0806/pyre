...
pub enum OpKind {
    ...
    ///
    /// RPython upstream emits four distinct opnames in the
    /// guard_value family via `jit_codewriter/jtransform.py:608-648
    /// rewrite_op_hint`:
    ///
    ///   * `int_guard_value` / `ref_guard_value` / `float_guard_value`
    ///     (jit.py:611) — pointer/value compare per the arg's
    ///     `getkind()`.
    ///   * `str_guard_value` (jit.py:631 **and** :647) — value-equality
    ///     compare for BOTH `promote_string` (`OS_STREQ_NONNULL`) and
    ///     `promote_unicode` (`OS_UNIEQ_NONNULL`).  The two share a
